...
const ANALIZAR_TRACKING = (LISTADO_TRACKING, LISTADO_PC) => {
    ...
    MOVILES.forEach(element => {        
        const TR_FILTRADOS = LISTADO_TRACKING.filter((item) => item.PPU === element);
        PASADAS_PC = LISTAR_PASADAS(TR_FILTRADOS, LISTADO_PC);
        TRACK_MOVILES[element] = FILTRAR_INTERPOLACIONES(LISTADO_PC, PASADAS_PC);//APLANANDO
        PASADAS_PC = null;
    });
        ...
        if(track.CODI_SENTI !== "-1") {
            if(TR_FILTRADOS[i+1]){
                ...
                if(TRACK_ACT.PPU, TRACK_POS.PPU){
                    if(TRACK_ACT.CODI_SERVI === TRACK_POS.CODI_SERVI && TRACK_ACT.CODI_SENTI === TRACK_POS.CODI_SENTI){  
                        let PC_SERVICIO = PC_X_SERVICIO(LISTADO_PC, TRACK_ACT.NOMB_SERVI, TRACK_ACT.CODI_SENTI);
                        PC_SERVICIO.forEach((punto) => {
                            ...
                            if(maxDistEntrePtosGPS(COOR_ACT, COOR_POS) && maxTiempoEntrePtosGPS(TRACK_ACT.HORA_CHL, TRACK_POS.HORA_CHL)){
                                let HORA_INTERPOLADA = GET_FECHA_INTERPOLADA(TRACK_ACT, TRACK_POS);
                                LA_PASADA.PASO_HORA_CHL = HORA_INTERPOLADA;
                                LA_PASADA.PASO_HORA_UTC = moment(HORA_INTERPOLADA).add(4, 'hours').format("DD/MM/YYYY HH:mm:ss"); //-04:00;
                                let DISTACIA = parseInt(geolib.getDistanceFromLine(COOR_PC, COOR_ACT, COOR_POS));
                                if(DISTACIA <= 130 && INTERPOLARE){
                                    LA_PASADA.V_INTERPOLADA = GET_VELOCIDAD_INTERPOLADA(COOR_ACT, COOR_POS, TRACK_ACT.HORA_CHL, TRACK_POS.HORA_CHL, "kmh");
...
const FILTRAR_INTERPOLACIONES = (LISTADO_PC, PASADAS) => {
    ...
    LISTADO_PC.forEach(element => {
        const PASADAS_CORRELATIVAS = PASADAS.filter(pasada => pasada.CORRELATIVO === element.POSI_PC && pasada.NOMB_SERVI === element.NOMB_SERVI)
...
const PROCESAR_LISTADO_PASADAS = (TRACK_MOVILES) => {
    ...
    fs.writeFileSync(JSON.stringify(TRACK_MOVILES, ' ', 3), 'ejemplo.json');
...
const PC_X_SERVICIO = (LISTADO_PC, NOMB_SERVI, CODI_SENTI) => {
    let PC_SERVICIO = [];
    PC_SERVICIO = LISTADO_PC.filter(item => (item.NOMB_SERVI === NOMB_SERVI && item.CODI_SENTI == CODI_SENTI));
...
const GET_FECHA_INTERPOLADA = (TRACK_ACT, TRACK_POS) => {
    ...
    FECHA_INTERPOLADA = moment(T1, "DD/MM/YYYY HH:mm:ss").format("DD/MM/YYYY HH:mm:ss");
...
LEER_PC(URL_PC);
LEER_TRACKING(URL_TRACKING);

setTimeout(() => {
    if(LISTADO_TRACKING.length > 0) {
        let SERVICIOS = [];
        const TR_FILTRADOS = LISTADO_TRACKING.filter((item) => item.PPU === "ZP9889");
        // const TR_FILTRADOS = LISTADO_TRACKING.filter((item) => item.PPU === "JXWH40");
        const PC_FILTRADOS = LISTADO_PC.filter((item) => item.NOMB_SERVI === "104N" && item.CODI_SENTI === "1");
        // ANALIZAR_TRACKING(TR_FILTRADOS, PC_FILTRADOS);
